...
def generate(api, plans, combat, runtime):
    ...
    api.set_cells(tell, skills, {'skill': warning, '*Id': str(warning_id),
        ...
        'anim': 'SC', 'monanim': 'S1', 'InGame': '1', '*eol': '0'})
    skills.append(tell)
    ...
    api.set_cells(row, skills, {'skill': nova, '*Id': str(nova_id),
        ...
        'srvmissilea': nova, 'cltmissilea': nova, 'monanim': 'S1',
    ...
    for plan in plans:
        ...
        boss_rooms.add_skill(api, monsters, boss, warning, 'S1')
        boss_rooms.add_skill(api, monsters, boss, nova, 'S1', plan['tier'])
